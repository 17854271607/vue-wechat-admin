...
<script>
	import { Power, Icon } from '@/api/index'
	export default {
		data() {
			return {
				defaultProps: {
		...
		methods: {
			...
			async showIcon(node, data) {
				...
				let { status, icons } = await Icon.getIcon({ pageSize: 30, pageIndex: 1 })
				if (status) {
					this.iconForm = icons;
			...
			async handleCurrentChange(val) {
				...
				let { status, icons } = await Icon.getIcon({ pageSize: size, pageIndex: val });
				if (status) {
					this.iconForm = icons
					this.iconForm.forEach((item) => {
						item.name = 'el-icon-' + item.name
			...
			async handleIcon() {
				...
				let { status, msg } = await Icon.icon(this.iconAddForm);
				if (status) {
			...
			async loadNode(node, resolve) {
				...
				if (node.level == 0) {
					let { satus, data } = await Power.menuInfo({ pId: 0 });
					return resolve(data);

				}
				//父级节点id
				let pId = node.data.id;
				let { status, data } = await Power.menuInfo({ pId });
				if (status) {
			...
			//编辑提交
			async submitEdit() {
				let { status } = await Power.editMenu({ ...this.editForm });
				if (status) {
			...
			//添加提交
			async submitAdd() {
				let { status, data } = await Power.insertMenu({ ...this.insertForm });
				if (status) {
			...
			remove(node, data) {
				...
				this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
					...
					let { status } = await Power.delMenu({ id });
					if (status) {
